...
for w_kghr in w_kghr_a:
    p1 = choke.p_choke_up_gasoil_atm(w_kghr, p2_atm, 1, 0.55)
    p2 = choke.p_choke_up_go_atm(w_kghr, p2_atm, 1)
    p3 = choke.p_choke_up_goil_atm(w_kghr, p2_atm, 1)
    q1 = np.append(q1, p1)
    q2 = np.append(q2, p2)
    q3 = np.append(q3, p3)
...
q3 = np.array([])
for w_kghr in w_kghr_a:
    p1 = choke.p_choke_down_gasoil_atm(w_kghr, p1_atm, 1, 0.55)
    p2 = choke.p_choke_down_go_atm(w_kghr, p1_atm, 1)
    p3 = choke.p_choke_down_goil_atm(w_kghr, p1_atm, 1)
    q1 = np.append(q1, p1)
    q2 = np.append(q2, p2)
    q3 = np.append(q3, p3)
plt.plot(w_kghr_a, q3, label='Перкинс')
