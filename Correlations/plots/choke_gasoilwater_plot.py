...
q3 = np.array([])
for p2_atm in p2_a:
    p3 = choke.W_choke_gasoilwater_kghr(p1_atm, p2_atm, 1, T_C=20, d2_mm=5)
    q3 = np.append(q3, p3)
...
for w_kghr in w_kghr_a:
    p3 = choke.p_choke_up_gasoilwater_atm(w_kghr, p2_atm, 1)
    q3 = np.append(q3, p3)
...
q3 = np.array([])
for w_kghr in w_kghr_a:
    p3 = choke.p_choke_down_gasoilwater_atm(w_kghr, p1_atm, 1)
    q3 = np.append(q3, p3)
plt.plot(w_kghr_a, q3, label='Перкинс')
